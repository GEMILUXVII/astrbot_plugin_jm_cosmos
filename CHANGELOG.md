...
### 安全与健壮性修复
- **并发安全**：各操作与下载改用独立的 jmcomic client（`new_jm_client`），避免共享 client 在并发线程下竞态；登录后通过 `update_cookies` 注入会话
- **凭据保护**：`/jmlogin` 在群聊中被拒绝，避免账号密码泄露到群消息/历史/日志
- **配额原子化**：下载前原子预留配额并在失败时返还，修复并发 TOCTOU 超发；管理员不再被计入配额
- **加密失败关闭**：设置了打包密码但缺少 `pyzipper` 时直接报错，不再静默产出未加密 ZIP
- **错误归类**：搜索/详情的网络错误不再被吞为"未找到"，由 `classify_exception` 区分网络失败与本子不存在
