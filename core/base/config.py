...
class JMConfigManager:
    ...
    def http_file_server_enabled(self) -> bool:
        ...
        return self.plugin_config.get("http_file_server_enabled", False)

    @property
    def http_file_server_bind_host(self) -> str:
        """Address used by the HTTP server for binding."""
        value = self.plugin_config.get("http_file_server_bind_host", "0.0.0.0")
        return str(value or "0.0.0.0").strip()

    @property
    def http_file_server_public_host(self) -> str:
        """Host name or IP that the protocol adapter can reach."""
        value = self.plugin_config.get("http_file_server_public_host", "127.0.0.1")
        return str(value or "127.0.0.1").strip()

    @property
    def http_file_server_port(self) -> int:
        """Port used by the HTTP file server."""
        port = int(self.plugin_config.get("http_file_server_port", 8639) or 8639)
        if not 1 <= port <= 65535:
            raise ValueError("HTTP file server port must be between 1 and 65535")
        return port
